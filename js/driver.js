/**
 * MAIN APP CONTROLLER - SISI KURIR / DRIVER (BULLETPROOF VERSION)
 * - Navbar Anti-Error
...
function setupNavbarManager() {
    ...
    window.switchTab = function(target) {
        ...
        if (target === 'map' && typeof myMap !== 'undefined' && myMap && myMap.map) {
            setTimeout(() => myMap.map.invalidateSize(), 300);
        }
    };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        if (typeof DynamicMap !== 'undefined') {
            myMap = new DynamicMap('map', OFFICE.lat, OFFICE.lng, 15);
            if (myMap && myMap.map) {
                myMap.map.touchZoom.enable();
                myMap.map.dragging.enable();
                if (myMap.map.tap) myMap.map.tap.disable();

                const tileStreet = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 });
                ...
                if (btnMapLayer) {
                    btnMapLayer.onclick = () => {
                        if (isSatMode) { myMap.map.removeLayer(tileSat); tileStreet.addTo(myMap.map); } 
                        else { myMap.map.removeLayer(tileStreet); tileSat.addTo(myMap.map); }
                        isSatMode = !isSatMode;
                    };
                }

                const officeIcon = L.icon({ iconUrl: '/assets/icons/pin.png', iconSize: [45, 45], iconAnchor: [22.5, 45] });
                ...
                myMap.map.on('locationfound', (e) => { 
                    ...
                    if (!driverMarker) {
                        ...
                        driverMarker = L.marker([e.latlng.lat, e.latlng.lng], { icon: kurirIcon, zIndexOffset: 9999 }).addTo(myMap.map);
                ...
                if (btnMyLocation) btnMyLocation.onclick = () => myMap.map.setView(currentGpsLocation, 17, { animate: true });
            }
        }
    ...
    if (session) {
        ...
        if (profile && (profile.role === 'driver' || profile.role === 'kurir')) {
            ...
            validSession = {
                id: profile.id, nama: profile.full_name || 'Driver Mapel', whatsapp: profile.whatsapp || '-',
                nopol: profile.nopol || '', tipe_kendaraan: profile.tipe_kendaraan || 'Motor', warna: profile.warna || '', foto: profile.foto || '/assets/icons/kurir.png'
            };
        }
    }
    if (!validSession) {
        const localData = localStorage.getItem('mapel_driver_session');
        if (localData) { window.driverSessionSource = 'local'; validSession = JSON.parse(localData); }
    }
    if (!validSession) {
    ...
    // Tombol Simulator Tes Order (Buat Debugging)
    const btnTest = document.getElementById('btn-test-order');
    if(btnTest) {
        btnTest.style.display = 'block'; 
        btnTest.onclick = () => {
            if(!isOnline) return alert("Driver harus ONLINE dulu bos!");
            munculkanPopupOrder({
                id: 9999,
                data: { alamatJemput: 'Kantor Pusat Ekspedisi', alamatTujuan: 'Jalan Raya Test No. 1', keterangan: 'Paket Mockup', berat: '1kg', totalOngkir: 12000, jemputLat: OFFICE.lat + 0.005, jemputLng: OFFICE.lng + 0.005, tujuanLat: OFFICE.lat - 0.005, tujuanLng: OFFICE.lng - 0.005 }
            });
        };
    }
    ...
    if (btnSaveProfile) {
        btnSaveProfile.onclick = async () => {
            ...
            const updateData = {
                nama: document.getElementById('set-nama').value, whatsapp: document.getElementById('set-wa').value,
                nopol: document.getElementById('set-nopol').value, warna: document.getElementById('set-warna').value,
                tipe_kendaraan: document.getElementById('set-jenis').value, foto: window.driverSession.foto 
            };
            try {
                if (window.driverSessionSource === 'auth') {
                    await sb.from('profiles').update({ full_name: updateData.nama, whatsapp: updateData.whatsapp, nopol: updateData.nopol, warna: updateData.warna, tipe_kendaraan: updateData.tipe_kendaraan, foto: updateData.foto }).eq('id', window.driverSession.id);
                } else {
            ...
            finally { btnSaveProfile.innerText = "SIMPAN PERUBAHAN"; btnSaveProfile.disabled = false; }
        };
    }
});
    ...
    try {
        ...
        if (eksData && eksData.length > 0) {
            eksData.forEach(eks => {
                ...
                myMap.addMarker(eks.lat, eks.lng, 'ekspedisi', `<div class="text-center font-sans"><p class="font-bold text-[13px] text-gray-800">${eks.nama}</p></div>`, `/assets/icons/pin.png`);
            });
        }
...
function listenToIncomingOrders() {
    ...
    driverChannel
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, payload => {
            ...
            console.log("Status Realtime DB:", status);
        });
}
...
function munculkanPopupOrder(order) {
    ...
    document.querySelector('.text-[16px].font-black.text-blue-700').innerText = `Rp ${d.totalOngkir ? d.totalOngkir.toLocaleString('id-ID') : 0}`;
    ...
    document.getElementById('btn-terima').onclick = async () => {
        ...
        const driverInfo = { name: window.driverSession.nama, wa: window.driverSession.whatsapp, nopol: window.driverSession.nopol, photo: window.driverSession.foto };
        const updatedData = { ...d, driver: driverInfo };

        if(order.id === 9999) { // Logika kalau ini cuma order bohongan dari tombol test
             activeOrder = { id: order.id, ...updatedData };
             popup.classList.add('opacity-0'); card.classList.add('scale-95');
             setTimeout(() => popup.classList.add('hidden'), 300);
             document.getElementById('btn-terima').innerText = 'TERIMA ORDER';
             return mulaiTugasDriver();
        }
...
document.getElementById('input-kamera').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(event) {
            ...
            btnSubmit.dataset.base64 = event.target.result;
        }
...
document.getElementById('btn-submit-foto').onclick = async function() {
    ...
    if(photoStep === 'pickup') {
        if(activeOrder.id !== 9999) await sb.from('orders').update({ status: 'picked_up', data: {...activeOrder, pickup_photo: base64Foto} }).eq('id', activeOrder.id);
        window.tutupKamera();
        ...
        btnAction.className = "w-full bg-green-500 text-white font-bold py-4 px-6 rounded-2xl shadow-lg active:bg-green-600 transition-all mt-2";
        ...
        if(activeOrder.id !== 9999) await sb.from('orders').update({ status: 'completed', data: {...activeOrder, dropoff_photo: base64Foto} }).eq('id', activeOrder.id);
        
        driverStats.saldo += activeOrder.totalOngkir || 0;
        ...
        if(myMap && myMap.routingControl) myMap.map.removeControl(myMap.routingControl);
